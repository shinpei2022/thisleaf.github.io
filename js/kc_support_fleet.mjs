		...
		this.e_deckbuilder_div = NODE(ELEMENT("div", "", "option"), [
			...
			NODE(ELEMENT("div", "", "option_text"), [
				HTML(
					...
					+ " さんで扱われるデッキビルダー形式データを、支援艦隊メンバーとして反映します(未対応の艦は空欄になります)<br>"
					+ "2艦隊同時に読み込んだ場合、装備優先度は両方とも同じ(3,3)になります。決戦支援側の優先度を上げるか前衛支援側の優先度を下げてください。(3,6)など。"
				),
			]),
		]),
